...
data$Init.time <- data$Init.time / data$DocSize
data$Update.time <- data$Update.time / data$DocSize
...
pT <-   ggplot(dataT, aes(x=DocSize,
        ...
        #geom_point(position=pd, size=3, shape=21, fill="white") + # 21 is filled circle
        xlab("Data size (documents)") +
        ylab("Run time (seconds per document)") +
        geom_point(size=5) +
...
embed_fonts("nb_plot.pdf", outfile="plots/nbplottimes2.pdf")
